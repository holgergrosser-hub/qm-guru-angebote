  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        throw new Error(result.message || 'Fehler beim Verarbeiten');
      }

    } catch (err) {
      setStatus('error');
      setErrorMsg('Es gab einen Fehler beim Versenden. Bitte versuchen Sie es später erneut.');
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="mb-8">
            ...
            <textarea
              ...
              rows="4"
              className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 transition resize-none"
            />
          </div>
          ...
          {/* Download Link (wenn erfolgreich) */}
          {status === 'success' && downloadLink && (
            <div className="mt-6 p-4 bg-green-50 border-2 border-green-300 rounded-lg">
              ...
              <div className="space-y-2">
                <p className="text-sm text-green-700">📧 Eine Email mit dem PDF wird in Kürze zugestellt</p>
                <p className="text-sm text-green-700">💾 Oder laden Sie das Angebot direkt herunter:</p>
                <a
                  href={downloadLink}
                  target="_blank"
          ...
          <div className="mt-8 p-4 bg-blue-50 border-l-4 border-blue-600 rounded">
            <p className="text-sm text-blue-800">
              <strong>💡 Hinweis:</strong> Ihr Angebot wird in Echtzeit generiert und sofort an Ihre Email versendet. Sie können es auch direkt downloaden und haben es zu 100% transparent.
            </p>
          </div>
        </form>
